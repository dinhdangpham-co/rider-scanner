...
const TG_REPORT_INTERVAL = 13 * 60 * 1000; // 13 min — send summary before Render sleeps at 15min
...
// ===== TELEGRAM =====
// Fix Vietnamese encoding issues (replacement chars)
function fixVN(s){return(s||'').replace(/�/g,'').replace(/\s+/g,' ').trim()}

function sendTG(text){
  if(!TG_TOKEN||!TG_CHAT)return;
  const body=JSON.stringify({chat_id:TG_CHAT,text:fixVN(text),parse_mode:'HTML'});
...
// ===== TO COMPARISON (enhanced — includes rider info + xa detection) =====
...
function buildReport(hubId) {
  const hub = getHub(hubId);
  if (!hub) return null;
  const today = vnToday();
  const ts = (hub.scans||[]).filter(s=>s.timestamp&&s.timestamp.startsWith(today)&&s.scan_type==='PARCEL');
  const byR={};ts.forEach(s=>{byR[s.rider_name]=(byR[s.rider_name]||0)+1});
  const toComp = compareTO(hubId);
  const missingTOs = toComp.filter(t=>t.status==='missing'&&!t.isXa);
  const extraTOs = toComp.filter(t=>t.status==='extra');
  const xaTOs = toComp.filter(t=>t.isXa);
  const totalMissing = missingTOs.reduce((s,t)=>s+Math.abs(t.diff),0);
  const idleRiders = (hub.riders||[]).filter(r=>!byR[r.name]);

  let msg = `📊 <b>SPX Scanner — ${hub.name||hubId}</b>\n`;
  msg += `🕐 ${vnNow().slice(11,19)}\n\n`;
  msg += `📦 <b>${ts.length}</b> don da quet\n`;
  msg += `🏍️ <b>${Object.keys(byR).length}/${(hub.riders||[]).length}</b> rider active\n`;
  msg += `📋 <b>${toComp.length}</b> TO | ${xaTOs.length} xa\n`;
  if (totalMissing > 0) msg += `⚠️ <b>${totalMissing}</b> don thieu (${missingTOs.length} TO)\n`;
  if (extraTOs.length > 0) msg += `🔴 <b>${extraTOs.length}</b> TO thua don\n`;
  if (idleRiders.length > 0) msg += `😴 <b>${idleRiders.length}</b> rider chua quet\n`;

  // Top riders
  const topR = Object.entries(byR).sort((a,b)=>b[1]-a[1]).slice(0,5);
  if (topR.length) { msg += '\n🏆 Top:\n'; topR.forEach(([n,c])=>{msg+=`  ${n.split(' ').slice(-2).join(' ')}: ${c}\n`}); }
...
const server = http.createServer(async(req,res)=>{
  ...
  if(p==='/api/hubs'&&req.method==='POST'){
    ...
    saveDB(db);
    // ★ Save to master sheet registry (persists across restarts)
    saveHubToRegistry(body.id).catch(e=>{});
  ...
  if(p==='/api/hubs/update'&&req.method==='POST'){
    ...
    saveDB(db);saveHubToRegistry(body.id).catch(e=>{});return json(res,{success:true});
  }
  ...
  // ── CONFIG ──
  if(p==='/api/config'&&req.method==='GET'){const hub=getHub(hubId);return json(res,{success:true,data:{hub_id:hubId,hub_name:hub?.name||'',gas_url:hub?.gas_url||'',gas_key:hub?.gas_key||'',groq_key:db.config.groq_key?'***set***':'',master_count:hub?.master_data?.length||0,rider_count:hub?.riders?.length||0,user_count:db.users.length,scan_count:hub?.scans?.length||0,raw_to_count:hub?.raw_to?.length||0,sync:hub?.sync||{},backup_last:db.config.backup_last||''}})}
  if(p==='/api/config'&&req.method==='POST'){
    const body=await readBody(req);const hub=getHub(hubId);
    if(hub){if(body.gas_url!==undefined)hub.gas_url=body.gas_url;if(body.gas_key!==undefined)hub.gas_key=body.gas_key;}
    if(body.groq_key!==undefined)db.config.groq_key=body.groq_key;
  ...
  if(p==='/api/ai/chat'&&req.method==='POST'){
    ...
    const byR={};ts.forEach(s=>{byR[s.rider_name]=(byR[s.rider_name]||0)+1});
    const toComp=compareTO(hubId);
    const context=`Hub: ${hub?.name||hubId}. Today: ${ts.length} scans, ${Object.keys(byR).length}/${(hub?.riders||[]).length} riders active. TO comparison: ${toComp.filter(t=>t.status==='missing').length} missing, ${toComp.filter(t=>t.status==='extra').length} extra. Top riders: ${Object.entries(byR).sort((a,b)=>b[1]-a[1]).slice(0,5).map(([n,c])=>n.split(' ').slice(-2).join(' ')+':'+c).join(', ')}. Idle riders: ${(hub?.riders||[]).filter(r=>!byR[r.name]).length}.`;

    const GROQ_KEY=process.env.GROQ_API_KEY||db.config.groq_key||'';
    try{
      const groqBody=JSON.stringify({model:'llama-3.1-8b-instant',messages:[{role:'system',content:'You are SPX Scanner AI assistant for logistics hub operations. Answer in the same language as the user (Vietnamese or English). Be concise. Current data: '+context},{role:'user',content:body.message}],max_tokens:500,temperature:0.3});
      const groqResp=await new Promise((ok,no)=>{
        const req=https.request({hostname:'api.groq.com',path:'/openai/v1/chat/completions',method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+GROQ_KEY,'Content-Length':Buffer.byteLength(groqBody)}},res=>{let d='';res.on('data',c=>d+=c);res.on('end',()=>ok(d));res.on('error',no)});
        req.on('error',no);req.on('timeout',()=>no(new Error('Timeout')));req.write(groqBody);req.end();
      });
      const parsed=JSON.parse(groqResp);
      const reply=parsed.choices?.[0]?.message?.content;
      if(reply) return json(res,{success:true,reply,source:'groq'});
      throw new Error('No AI response');
    }catch(e){
      // Fallback: smart local response
      const q=(body.message||'').toLowerCase();
      let reply='';
      if(q.includes('rider')&&q.includes('idle')||q.includes('chua quet')){reply=`Hiện có ${(hub?.riders||[]).filter(r=>!byR[r.name]).length} rider chưa quét hôm nay.`}
      else if(q.includes('thieu')||q.includes('missing')){const m=toComp.filter(t=>t.status==='missing'&&!t.isXa);reply=`Có ${m.length} TO thiếu đơn, tổng ${m.reduce((s,t)=>s+Math.abs(t.diff),0)} đơn thiếu.`}
      else if(q.includes('tong')||q.includes('summary')||q.includes('bao cao')){reply=`Hôm nay: ${ts.length} đơn đã quét, ${Object.keys(byR).length} rider active, ${toComp.length} TO.`}
      else{reply=`Hôm nay hub ${hub?.name||hubId}: ${ts.length} đơn, ${Object.keys(byR).length} riders. Hỏi tôi về rider, TO, thiếu đơn...`}
      return json(res,{success:true,reply,source:'local'});
    }
...
// ===== LOAD HUB REGISTRY from master sheet on startup =====
const MASTER_GAS = 'https://script.google.com/macros/s/AKfycbz9uDCW96ctv_jFGaQzuQaR6uJgJXdlN3O7JljuSI5rQGKtEOHg7lYJzE4DSz24tCos/exec';
const MASTER_KEY = 'spx-scanner-2026';

async function loadHubRegistry() {
  try {
    const sep = MASTER_GAS.includes('?') ? '&' : '?';
    const raw = await fetchUrl(`${MASTER_GAS}${sep}action=getHubRegistry&key=${encodeURIComponent(MASTER_KEY)}`);
    const data = JSON.parse(raw);
    if (data.data && data.data.length > 0) {
      const find = (...keys) => Object.keys(data.data[0]).find(h => keys.some(k => h.toLowerCase().includes(k.toLowerCase()))) || '';
      data.data.forEach(row => {
        const id = (row[find('HubID','Hub ID','hub_id')] || '').trim();
        const gasUrl = (row[find('GAS_URL','gas_url','Gas URL')] || '').trim();
        if (!id || !gasUrl) return;
        if (!db.hubs[id]) {
          db.hubs[id] = { name: row[find('HubName','Hub Name','name')] || id, gas_url: gasUrl, gas_key: row[find('GAS_Key','gas_key','Key')] || 'spx-scanner-2026', riders: [], scans: [], master_data: [], raw_to: [], sync: { last: '', status: '', error: '' } };
          console.log(`[Registry] Loaded hub: ${id}`);
        } else {
          // Update GAS URL if changed
          db.hubs[id].gas_url = gasUrl;
          db.hubs[id].name = row[find('HubName','Hub Name','name')] || db.hubs[id].name;
        }
      });
      saveDB(db);
    }
    console.log(`[Registry] Hub registry loaded: ${hubIds().length} hubs`);
  } catch(e) { console.error('[Registry] Error:', e.message); }
}

// Save hub to master sheet registry when added/updated
async function saveHubToRegistry(hubId) {
  const hub = getHub(hubId);
  if (!hub) return;
  try {
    const body = JSON.stringify({ action: 'saveHubRegistry', key: MASTER_KEY, data: { id: hubId, name: hub.name, gas_url: hub.gas_url, gas_key: hub.gas_key } });
    const u = new URL(MASTER_GAS);
    const opts = { hostname: u.hostname, path: u.pathname, method: 'POST', timeout: 15000, headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) } };
    const req = https.request(opts, res => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) { res.resume(); fetchUrl(res.headers.location).catch(e => {}); }
      else { let d = ''; res.on('data', c => d += c); res.on('end', () => {}); }
    });
...
server.listen(PORT,'0.0.0.0',()=>{
  console.log(`\n📦 SPX Scanner v8.7 Multi-Hub | http://0.0.0.0:${PORT} | VN UTC+7`);
  console.log(`   Hubs: ${hubIds().join(', ')} | TG: ${TG_TOKEN?'✅':'❌'}\n`);
  // ★ Load hub registry from master sheet FIRST, then sync all
  setTimeout(async ()=>{
    await loadHubRegistry().catch(e=>console.error('[Registry]',e.message));
    await syncAllHubs().catch(e=>console.error('[Sync]',e.message));
  },1000);
  // Periodic sync
  setInterval(()=>syncAllHubs().catch(e=>{}),SYNC_INTERVAL);
  // Hourly backup
  setInterval(()=>backupAll().catch(e=>{}),BACKUP_INTERVAL);
  // ★ Telegram auto report every 13 min (before Render 15min sleep)
  setInterval(()=>sendAutoReport(),TG_REPORT_INTERVAL);
  // Send first report 2 min after start
  setTimeout(()=>sendAutoReport(),120000);
});

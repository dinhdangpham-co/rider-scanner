...
function parseTSV(raw) {
  ...
  for(let i=1;i<lines.length;i++){
    ...
    const obj={};hdr.forEach((h,j)=>obj[h]=cols[j]||'');
    rows.push(obj);
  }
  return rows;
}
...
function gasGet(action) {
  ...
  if (!gasUrl) return Promise.reject(new Error('GAS_URL chưa cấu hình'));
  const key = db.config.gas_key || GAS_KEY;
  // ★ Handle both /exec URLs and redirected /echo URLs
  const sep = gasUrl.includes('?') ? '&' : '?';
  const url = `${gasUrl}${sep}action=${action}&key=${encodeURIComponent(key)}`;
  return fetchUrl(url);
}
...
const server = http.createServer(async(req,res)=>{
  ...
  // ── MASTER DATA (manual upload + TSV URL fallback) ──
  ...
  // ★ TSV URL fallback — load master data from published Google Sheets TSV
  if(p==='/api/master/tsv'&&req.method==='POST'){
    const body=await readBody(req);
    const url=body.url;
    if(!url)return json(res,{success:false,error:'URL required'},400);
    try{
      const raw=await fetchUrl(url);
      const rows=parseTSV(raw);
      if(!rows.length)return json(res,{success:false,error:'No data'},400);
      const hdr=Object.keys(rows[0]);
      const find=(...keys)=>hdr.find(h=>keys.some(k=>h.toLowerCase().includes(k.toLowerCase())))||'';
      db.master_data=rows.map(r=>({
        tracking:r[find('Order ID')]||r[find('SPX TN')]||r[find('tracking')]||'',
        sls_tn:r[find('SLS Tracking')]||r[find('sls_tn')]||'',
        zone:r[find('Zone ID')]||r[find('Zone')]||'',
        sort_code:r[find('Sort Code')]||'',
        ward:r[find('Ward')]||'',
        status:r[find('Status')]||'',
        driver_name:r[find('Driver Name')]||'',
        to_code:r[find('TO Number')]||r[find('to_code')]||'',
        weight:r[find('Weight')]||'',
      }));
      if(body.save_url)db.config.tsv_url=url;
      saveDB(db);

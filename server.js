...
// ===== GAS per hub =====
function gasGet(hubId, action) {
  const hub = getHub(hubId);
  if (!hub || !hub.gas_url) return Promise.reject(new Error('Hub not configured'));
  const sep = hub.gas_url.includes('?') ? '&' : '?';
  return fetchUrl(`${hub.gas_url}${sep}action=${action}&key=${encodeURIComponent(hub.gas_key||'spx-scanner-2026')}`);
}
  ...
  try {
    ...
    try {
      ...
      if (tData.data && tData.data.length > 0) {
        ...
        console.log(`[Sync] ${hubId} RawN: ${hub.raw_to.length}`);
      }
    } catch(e) { console.log(`[Sync] ${hubId} RawN skip:`, e.message); }
...
const server = http.createServer(async(req,res)=>{
  ...
  if(p==='/api/sync/status'&&req.method==='GET'){const hub=getHub(hubId);return json(res,{success:true,data:hub?{...hub.sync,master_count:hub.master_data?.length||0,rider_count:hub.riders?.length||0,raw_to_count:hub.raw_to?.length||0}:{}})}
...
server.listen(PORT,'0.0.0.0',()=>{
  console.log(`\n📦 SPX Scanner v8 Multi-Hub | http://0.0.0.0:${PORT} | VN UTC+7`);
  console.log(`   Hubs: ${hubIds().join(', ')} | TG: ${TG_TOKEN?'✅':'❌'}\n`);
  // Auto-sync all hubs on startup
  setTimeout(()=>syncAllHubs().catch(e=>console.error('[Sync]',e.message)),5000);
  // Periodic sync
